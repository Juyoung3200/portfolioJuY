...
$(document).ready(function(){
	...
	//검색한 질병과 연관된 타겟
	var target_tb = $('#targetList').DataTable({
...
//	타겟별 화합물 활성정보 -예측-
	...
	//전체 화합물에 대한 타겟 활성정보 -문헌- 
		...
		//예측 테이블 검색 설정 추가 부분
		$('<div id="condi_title" class="condition"><div class="condi_title"><span>검색 설정</span></div><div class="condi_check"><input id="ref_have" type="checkbox" checked><label for="ref_have"><span>문헌정보가 있는 활성정보</span></label><input id="ref_none" type="checkbox" checked><label for="ref_none"><span>문헌정보가 없는 활성정보</span></label></div><div class="line1"></div><div class="score"><p>associate score  <input id="min_score" type="text" placeholder=""> <span>이상  </span><input id="max_score" type="text" placeholder=""> <span>이하</span></p></div><div class="line2"></div>').insertBefore('#foreAllList_filter');
...
//	선택한 화합물이 포함된 천연물 정보 
	...
	$('.dataTables_filter input[type="search"]').attr("class", "searchBox");
	$('.searchBox').css({"width" : "400px"});


	//페이지 바뀔때 I프레임 리사이징
	$('#targetList').on('draw.dt',function(){
    	iframeResize('target_info',true);
	})
	...
	$('#orgList').on('draw.dt',function(){
    	iframeResize('org_info',true);
	})
	...
	// 전체 타겟에 대한 화합물 활성정보 (문헌)에서 화합물을 선택했을때

	$('.table5 tbody .select:nth-child(1)').click(function(){
		// 화합물 선택하면 배경색과 글씨색이 바뀐다.
		$(this).addClass('on')
		$('.table5 tbody .select:nth-child(2)').removeClass('on')
		$('.table5 tbody .select:nth-child(3)').removeClass('on')
		$('.table5 tbody .select:nth-child(4)').removeClass('on')
		$('.table5 tbody .select:nth-child(5)').removeClass('on')
		$('.table6 tbody .select:nth-child(1)').removeClass('on')
		$('.table6 tbody .select:nth-child(2)').removeClass('on')
		$('.table6 tbody .select:nth-child(3)').removeClass('on')
		$('.table6 tbody .select:nth-child(4)').removeClass('on')
		$('.table6 tbody .select:nth-child(5)').removeClass('on')
	});

	$('.table5 tbody .select:nth-child(2)').click(function(){
		// 화합물 선택하면 배경색과 글씨색이 바뀐다.
		$(this).addClass('on')
		$('.table5 tbody .select:nth-child(1)').removeClass('on')
		$('.table5 tbody .select:nth-child(3)').removeClass('on')
		$('.table5 tbody .select:nth-child(4)').removeClass('on')
		$('.table5 tbody .select:nth-child(5)').removeClass('on')
		$('.table6 tbody .select:nth-child(1)').removeClass('on')
	...
	$('.table5 tbody .select:nth-child(3)').click(function(){
		// 화합물 선택하면 배경색과 글씨색이 바뀐다.
		$(this).addClass('on')
		$('.table5 tbody .select:nth-child(1)').removeClass('on')
		$('.table5 tbody .select:nth-child(2)').removeClass('on')
	...
	$('.table5 tbody .select:nth-child(4)').click(function(){
		// 화합물 선택하면 배경색과 글씨색이 바뀐다.
		$(this).addClass('on')
		$('.table5 tbody .select:nth-child(1)').removeClass('on')
		$('.table5 tbody .select:nth-child(2)').removeClass('on')
	...
	$('.table5 tbody .select:nth-child(5)').click(function(){
		// 화합물 선택하면 배경색과 글씨색이 바뀐다.
		$(this).addClass('on')
		$('.table5 tbody .select:nth-child(1)').removeClass('on')
		$('.table5 tbody .select:nth-child(2)').removeClass('on')
	...
	// 전체 타겟에 대한 화합물 활성정보 (예측)에서 화합물을 선택했을때


	$('.table6 tbody .select:nth-child(1)').click(function(){
		// 화합물 선택하면 배경색과 글씨색이 바뀐다.
		$(this).addClass('on')
		$('.table6 tbody .select:nth-child(2)').removeClass('on')
		$('.table6 tbody .select:nth-child(3)').removeClass('on')
	...
	$('.table6 tbody .select:nth-child(2)').click(function(){
		// 화합물 선택하면 배경색과 글씨색이 바뀐다.
		$(this).addClass('on')
		$('.table6 tbody .select:nth-child(1)').removeClass('on')
		$('.table6 tbody .select:nth-child(3)').removeClass('on')
	...
	$('.table6 tbody .select:nth-child(3)').click(function(){
		// 화합물 선택하면 배경색과 글씨색이 바뀐다.
		$(this).addClass('on')
		$('.table6 tbody .select:nth-child(1)').removeClass('on')
		$('.table6 tbody .select:nth-child(2)').removeClass('on')
	...
	$('.table6 tbody .select:nth-child(4)').click(function(){
		// 화합물 선택하면 배경색과 글씨색이 바뀐다.
		$(this).addClass('on')
		$('.table6 tbody .select:nth-child(1)').removeClass('on')
		$('.table6 tbody .select:nth-child(2)').removeClass('on')
	...
	$('.table6 tbody .select:nth-child(5)').click(function(){
		// 화합물 선택하면 배경색과 글씨색이 바뀐다.
		$(this).addClass('on')
		$('.table6 tbody .select:nth-child(1)').removeClass('on')
		$('.table6 tbody .select:nth-child(2)').removeClass('on')
